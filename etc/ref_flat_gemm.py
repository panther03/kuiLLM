...
def load(stem="gemm_tc_flat_nosplitk_noepi", bf16=True,
         ...
         cs_m=None, cs_n=None, force_scalar_c=False, roll_v=False):
    ...
    patches out the epilogue's compile-time choice of a vectorized C read so
    that it takes its own scalar path, which is the path the Kuiper port
    implements; everything else in the translation unit is unchanged.
    """
    key = (stem, bf16, bm, bn, bk, wm, wn, skew, group, cs_m, cs_n,
           force_scalar_c, roll_v)
    if key in _cache:
        return _cache[key]
    ...
    if force_scalar_c or roll_v:
        text = cu.read_text()
        if force_scalar_c:
            needle = "constexpr bool C_CONTIGUOUS_ALIGNED_N ="
            i = text.index(needle)
            j = text.index(";", i)
            text = text[:i] + needle + " false" + text[j:]
        if roll_v:
            # The drain's vector-group loop. Unrolled, its 4 iterations put 4x
            # as many C loads in flight; rolled, it matches what KaRaMeL emits.
            needle = "        #pragma unroll\n        for (int v = lane;"
            assert text.count(needle) == 1
            text = text.replace(
                needle, "        #pragma unroll 1\n        for (int v = lane;")
