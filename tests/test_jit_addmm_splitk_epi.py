...
This backend chains two kernels on the stream with no synchronization between
them, so it is graph-safe and is an ordinary autotuning candidate. It is still
not the default pick -- ``supergemm`` comes first in the backend list -- so
both the default choice and reachability by name are checked here, along with
numeric agreement with ``torch.addmm`` at the shapes split-K exists for (skinny
M, large K).
...
def test_addmm_splitk_epi_selectable():
