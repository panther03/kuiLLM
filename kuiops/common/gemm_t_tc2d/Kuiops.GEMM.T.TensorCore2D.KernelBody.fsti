module Kuiops.GEMM.T.TensorCore2D.KernelBody

(* Fork of [Kuiper.Kernel.GEMM.TensorCore2D.To.KernelBody] for the
   transposed-B ("TN") GEMM.  Delta versus upstream:
     - [str_B] is an EXPLICIT [strided_col_major lB] argument
     - the shmem B tile is [cm bk bn]; the barrier is the [_cm] fork
     - [chunk et_ab /?+ bn] is gone: for TN the copy divisibility lands on
       [bk] only (proved by removing it and re-verifying)
   [To.KernelDesc], [To.KernelDesc.Teardown], [To.BaseSendable] and the whole
   [To.Epilogue*] chain are reused from upstream UNCHANGED -- all of them are
   already generic in [lB]. *)
