...
(* TN ("transposed-B") fork of [Kuiper.Kernel.GEMM.TensorCore2D.To.Finish].
   See the .fsti.  The only difference from upstream is that the shared-memory
   B tile is viewed as [l2_col_major bk bn] rather than [l2_row_major bk bn]
   (5 occurrences: [setup]'s return type and its [tensor_abs']/[from_array],
   and [sB]'s type in [cleanup] and [finish]).  The flat [SHArray et_ab
   (bk *^ bn)] allocation and every specification are unchanged. *)
    ...
    (exists* em2. FB.bp_sharing s._2 em2 nthr)
{
  ...
  tensor_abs' (cm bk bn) sarB;
  let sB = from_array (cm bk bn) sarB;
