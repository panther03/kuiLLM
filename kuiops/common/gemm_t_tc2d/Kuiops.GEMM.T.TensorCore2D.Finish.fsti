module Kuiops.GEMM.T.TensorCore2D.Finish

(* TN ("transposed-B") fork of [Kuiper.Kernel.GEMM.TensorCore2D.To.Finish].

   This is where the shared-memory B tile's layout is *chosen*: [setup] turns
   the flat shmem allocation [sarB] into an [array2].  Upstream views it as
   [l2_row_major bk bn]; here it is viewed as [l2_col_major bk bn].

   The underlying allocation is untouched.  [shmems_desc_to] declares the B tile
   as [SHArray et_ab (bk *^ bn)], a flat *element count*, and
   [tlayout_ulen (l2_col_major bk bn) == bk * bn == tlayout_ulen (l2_row_major bk bn)],
   so this is a layout-only change: no extra shared memory, no padding, no copy.

   Everything else -- the epilogue, the accumulator, [MS.mmcomb] -- is byte
   identical to upstream, and the B-free modules ([To.KernelDesc],
   [To.Epilogue], [To.EpilogueState], ...) are imported from upstream
   unchanged. *)
...
fn setup
  ...
  returns s :
    (array2 et_ab (rm bm bk) & array2 et_ab (cm bk bn))
  ensures
...
ghost fn cleanup
  ...
  (sB : array2 et_ab (cm bk bn) { core sB == fst (snd sh) })
  (tid : szlt nthr)
  (accFrags : array (fragment et_acc FragAcc tm tn tk FragLAcc))
