...
(* TN ("transposed-B") fork of
   [Kuiper.Kernel.GEMM.TensorCore2D.To.Fragments].  See the .fsti for the
   rationale.  The only differences from upstream are:
     - [fragarrayB_approximates] / [populate_fragments_b] / [fragarray_mma] /
       [subproducts_tc_2d] index B fragments by [FragLCM] instead of [FragLRM];
     - the B tile is viewed as [l2_col_major bk bn] instead of
       [l2_row_major bk bn];
     - [populate_fragments_b] calls [mma_loadB_cm] instead of [mma_loadB].
   Every specification (the [chest2]s, the [%~] approximations, [matmul]) is
...
let fragarrayB_approximates (#et:Type0) {| scalar et, real_like et |}
  (#tm #tn #tk : pos)
  (wn : nat)
  (arr : array (fragment et FragB tm tn tk FragLCM) { Pulse.Lib.Array.length arr == wn})
...
fn populate_fragments_b
  ...
  (bm bn bk
   tm tn tk
   wm wn : szp { constraints bm bn bk tm tn tk wm wn })
  (frags : array (fragment et FragB tm tn tk FragLCM))
  (gm : array2 et (cm bk bn))
  (#em : chest2 et bk bn)
