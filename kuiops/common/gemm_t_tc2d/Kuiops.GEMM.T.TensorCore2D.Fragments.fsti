module Kuiops.GEMM.T.TensorCore2D.Fragments

(* TN ("transposed-B") fork of
   [Kuiper.Kernel.GEMM.TensorCore2D.To.Fragments].

   The mathematical specification is *unchanged* from upstream: B is still a
   [chest2 real bk bn] and the product is still [matmul rA rB].  What changes is
   purely how B lives in memory and therefore how its tensorcore fragment is
   loaded:

     upstream: shmem B tile is [l2_row_major bk bn], fragment layout [FragLRM],
               loaded with [mma_loadB];
     here:     shmem B tile is [l2_col_major bk bn], fragment layout [FragLCM],
               loaded with [mma_loadB_cm].

   [mma_loadB] and [mma_loadB_cm] have identical postconditions
   ([fr |-> m0] with the same [chest2 et k n]) and differ only in the
   memory-layout precondition, and [mma_sync'] is already polymorphic in both
   operand fragment layouts with an unchanged spec.  So the multiply and the
   whole accumulator/epilogue chain are untouched.

   The fragment layout is a [Type]-level index (it occurs in [bFrags]'s own
   type), so this is a compile-time fork, never a runtime dispatch. *)
...
(* The B-free parts of the chain are imported from upstream unchanged. *)
