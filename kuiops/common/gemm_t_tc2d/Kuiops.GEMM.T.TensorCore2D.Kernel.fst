   ...
   warp_divides_thread_count bm bn tm tn wm wn nthr in
{
  ...
  barrier_contract = (fun bid ptrs -> FB.contract eA eB (rm bm bk) (cm bk bn) (fst ptrs) (fst (snd ptrs)) nthr bid);
  barrier_count    = (fun _bid -> 2 * (SZ.v k / SZ.v bk));
  barrier_ok = (fun bid ptrs -> FB.barrier_p_to_q_transform eA eB (rm bm bk) (cm bk bn) (fst ptrs) (fst (snd ptrs)) nthr bid);
