module Kuiops.Kernel.GEMM.TensorCore2D.To.KernelDesc.EpilogueChunkUpdate

#lang-pulse

open Kuiper
#set-options "--ifuel 1 --initial_fuel 0 --max_fuel 1 --z3rlimit 40"
...
open Kuiops.Kernel.GEMM.TensorCore2D.To.KernelDesc.EpilogueCellUpdate
...
fn epilogue_chunk_update
  ...
  (comb : et_cd -> et_acc -> et_cd)
  (#m #n : szp)
  (#lC : RO.vlayout2 m n) {| RO.cvtlayout lC |}
      ...
      1.0R (SZ.v row) (SZ.v col) (chunk et_cd) (reveal nv)
{
  ...
  assert pure ((chunk et_cd) /? SZ.v globalCol);
  ro_cell_aligned16 lC c globalRow globalCol;
  cell_aligned16 (rm m n) d globalRow globalCol;
    ...
    decreases (SZ.v (chunk et_cd) - SZ.v !k)
  {
    ...
    Pulse.Lib.Array.(obuf.(vk) <- comb cv av);
    with so'. assert obuf |-> so';
