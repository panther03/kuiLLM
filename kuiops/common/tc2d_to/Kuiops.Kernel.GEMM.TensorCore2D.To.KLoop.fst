module Kuiops.Kernel.GEMM.TensorCore2D.To.KLoop
...
open Kuiops.Kernel.GEMM.TensorCore2D.KernelDesc


open Kuiops.Kernel.GEMM.TensorCore2D.To.KernelDesc
open Kuiops.Kernel.GEMM.TensorCore2D.To.EpilogueState
open Kuiops.Kernel.GEMM.TensorCore2D.To.Fragments
open Kuiops.Kernel.GEMM.TensorCore2D.To.KLoopInvariant

(* Keep these small algebraic obligations out of the much larger [k_loop]
   context, where otherwise-equivalent inline lambdas can time out. *)
...
(* The flip-flop barrier contract is built over the raw arrays, while the
   barrier predicates use their two-dimensional views.  Normalize the
   contract projection here, outside the large loop-step proof. *)
    ...
    B.barrier_state (barrier_iteration vnext)
{
  ...
  bp_to_rin eA eB (rm bm bk) (rm bk bn) (core sA) (core sB)
    sA sB nthr bid (2 * v) tid;

  B.barrier_wait ();

  rout_to_bq eA eB (rm bm bk) (rm bk bn) (core sA) (core sB)
    sA sB nthr bid (2 * v) tid;
  ...
  lemma_writable_strided_row_major_stride_positive lA str_A #_;
  lemma_writable_strided_row_major_stride_positive lB str_B #_;
  lemma_to_kuiper_aligned_strided_row_major lA str_A (chunk et_ab);
  lemma_to_kuiper_aligned_strided_row_major lB str_B (chunk et_ab);
  copy_tiles_out_of_matrices_vec bm bn bk sA sB
    #_ #_ #(to_kuiper_strided_row_major lA str_A)
    #_ #_ #(to_kuiper_strided_row_major lB str_B)
    gA gB
    mrow v mcol
  ...
  Kuiper.Kernel.GEMM.TensorCore2D.FragmentAcc.subtile_approx
    eA rA bm bk mrow v;
  Kuiper.Kernel.GEMM.TensorCore2D.FragmentAcc.subtile_approx
    eB rB bk bn v mcol;
