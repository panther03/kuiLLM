module Kuiops.Kernel.GEMM.TensorCore2D.KernelDesc
...
(* [bid] splits into row and column block indices.  State the nonlinear bound
   explicitly so it remains cheap under split SMT queries. *)
...
{
  (* Bind these large specifications once.  Reusing the names avoids repeated
     elaboration and comparison of the same nested matrix expression. *)
