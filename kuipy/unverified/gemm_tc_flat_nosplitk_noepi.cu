...
//
//     D = A @ B^T,  one block per output tile.
//
...
//     A, B and D 16-byte aligned; K % VEC == 0 and N % VEC == 0
//
...
//        -c gemm_tc_flat_nosplitk_noepi.cu
// =============================================================================
...
extern "C" __global__ __launch_bounds__(THREADS) void gemm_tc_noepi_kernel(
    ...
    // ================================================================ epilogue
    // The accumulators have to become elem_t in global memory. Doing
    ...
    // -- the narrowing -- is plain arithmetic on plain floats, and the
...
extern "C" cudaError_t gemm_tc_flat_nosplitk_noepi_launch(
    ...
    if (SMEM_BYTES > 48 * 1024) {
        cudaError_t e = cudaFuncSetAttribute(
            gemm_tc_noepi_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize,
            SMEM_BYTES);
        if (e != cudaSuccess) return e;
    }

    dim3 grid((M / BM) * (N / BN));
    gemm_tc_noepi_kernel<<<grid, THREADS, SMEM_BYTES, stream>>>(
        A, B, D, M, N, K);
